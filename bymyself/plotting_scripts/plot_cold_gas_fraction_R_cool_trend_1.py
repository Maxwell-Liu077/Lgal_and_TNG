...
for i, snap in enumerate(snaps):
    ...
    num_eligible = np.count_nonzero(valid)
    ...
    ax.set_ylim(0, 1)
    ax.set_xlim(0, 5)
    ax.text(0.7, 0.1,f'z={z_snaps[i]:.2f}' + str(), bbox=dict(boxstyle="round", ec='lightgray', fc='white', alpha = 0.8), transform=ax.transAxes, size = 20)
